# Story 42.7 — End-to-End Regression Harness for Story 42
...
## User Story
As the product team, we need critical logging behavior protected by automated flows so visual refinements cannot silently break data integrity again.

## Required scenarios

### A — Untouched start
Start a fully prescribed workout; assert sets logged = 0 and no exercise complete.

### B — Quick-log strength
3×8 bench; enter weight; one commit; assert 3 correct session sets, exercise complete, card collapsed.

### C — Manual exception
Quick-log 3 sets, expand, change set 3 reps, assert other sets preserved and summary truthful.

### D — Bodyweight
Log reps; assert no weight requirement or fake 0 lb metric.

### E — Duration
Start 60-min walk: incomplete until actual duration committed.

### F — Distance + duration
Commit 5 mi + 30 min; assert completion and correct pace summary.

### G — Latency
Throttle endpoint; log exercise; immediately interact with next exercise; assert no blocking.

### H — Failure/retry
Fail one save; values preserved; state truthful; retry succeeds without duplicates.

### I — Reopen completed exercise
Complete, reopen during active workout, edit, recalculate metrics/completion.

### J — Completed-workout review
Finish workout; expand completed exercise; assert read-only data and absence of active mutation controls.

## Device matrix
- iPhone-size Safari mobile web
- desktop Chrome responsive viewport
- native iOS path when automation stack supports it

## Acceptance Criteria
- [ ] core scenarios automated or explicitly documented if technically blocked
- [ ] CI runs the domain-critical scenarios
- [ ] false-completion regression has a dedicated test
- [ ] fixtures are deterministic
- [ ] network latency/failure is deterministic
- [ ] tests validate persisted data and state, not screenshots only
- [ ] GitHub reviewer confirms meaningful domain assertions
...
## Claude Steering
Screenshots are supplemental. Assert session-set records, completion state, derived metrics, and rendered controls.
