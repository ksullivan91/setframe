# Story 42.4 — Batch + Optimistic Persistence for Workout Logging
...
## User Story
As a user logging several sets quickly, I want the interface to feel immediate and not wait on one API call before I continue.

## Existing Failure
Previous behavior populated child forms but still required per-set Save, and saves could block one another while requests completed.

## Product Intent
- local UI updates immediately
- one quick-log commit = one logical persistence operation
- user can continue to another exercise while save is in flight
- failures preserve entered data and remain recoverable

## Backend direction
Prefer a batch/transactional session-set mutation if practical. If not, concurrent per-set requests are acceptable temporarily, but they must be presented as one logical operation and handle partial failure explicitly. Do not serialize independent set saves.
...
## Acceptance Criteria
- [ ] Multiple sets can be logged without sequential waiting.
- [ ] Quick-log commit persists all affected sets.
- [ ] User never opens children solely to save them.
- [ ] UI updates optimistically after valid commit.
- [ ] User can work on another exercise during persistence.
- [ ] Failure preserves local values.
- [ ] Retry does not duplicate rows.
- [ ] Stale responses cannot overwrite newer edits.
- [ ] Partial failure is surfaced truthfully.
- [ ] Network latency/failure has deterministic tests.

## Error UX
Prefer concise inline recovery such as `Couldn't save 1 set — Retry`, without discarding user input.
...
## Claude Steering
Document the current API behavior before changing it. If a batch endpoint is needed, define the contract and migration. Do not call something “optimistic” if the UI remains blocked by the request.
