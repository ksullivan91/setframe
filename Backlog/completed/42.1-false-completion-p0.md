# Story 42.1 — P0: Stop False Exercise Completion on Workout Start
...
## User Story
As a user starting today's workout, I want every exercise to begin unlogged even when the template contains planned values, so Setframe only gives me credit for work I actually record.

## Screenshot / Gym-Test Evidence
The latest test shows Walk (treadmill) and Outdoor Cycle immediately in green completed states after only starting the workout from Today. No logging action occurred.

## Problem Statement
The implementation is conflating planned/default/session-generated values with actual logged performance. This is a data-integrity bug, not a styling issue.

## Required State Separation
Introduce or verify explicit semantic separation between planned, draft actual, saved actual, persistence status, and completion. Do not derive completion from non-null planned/default values.

## Acceptance Criteria
- [ ] Starting a fully prescribed workout produces 0 newly completed exercises.
- [ ] Starting a workout does not increment sets logged merely because session set rows were generated.
- [ ] Planned values remain visible.
- [ ] Planned values may seed editable drafts.
- [ ] Seeded drafts do not count as logged actuals.
- [ ] A set completes only after required representation-specific actual data is committed for the current session.
- [ ] Exercise completion derives from current-session completed sets, not template prescriptions.
- [ ] Bodyweight reps do not require a fake `0 lb`.
- [ ] RPE remains optional.
- [ ] Refreshing an untouched started workout remains incomplete.
- [ ] Refreshing after committed logging restores truthful saved state.
- [ ] Historical workouts and templates are not mutated or reclassified.
...
type SessionSet = {
  ...
  saveStatus: 'unlogged' | 'draft' | 'saving' | 'saved' | 'error';
};
```

Exact types may differ; the semantics may not.

## Required regression test

```text
Given a template with every planned value populated
When the user starts the workout
And performs no logging action
Then sets logged = 0
And no exercise is complete
And planned values remain visible
```
...
## Claude Steering
Trace template prescription → session creation → generated session sets → form defaults → persistence → completion selector. Fix the domain invariant before touching the visuals.
