# Story 42.5 — Exercise Completion, Auto-Collapse, Focus Handoff, and Reopen Lifecycle
...
## User Story
As I progress through a workout, I want exercises to become complete only when the required work is actually logged, then get out of my way automatically.

## Canonical completion rule
Exercise complete requires:
1. expected current-session sets exist
2. each required set has representation-required actuals
3. required actuals are committed according to Story 42.4
4. no blocking persistence error exists

Never complete from plan/default/generated-set existence.

## Auto-collapse
On incomplete → complete transition:
- provide brief rewarding feedback
- collapse details if open
- preserve compact completed card
- respect reduced-motion settings

## Handoff
When user intentionally moves to another exercise:
- validate/commit previous valid dirty quick-log state according to the agreed interaction
- collapse previous details
- do not falsely complete invalid data
- only open the new details if the user explicitly requests details

## Reopen in active workout
Completed exercise can be reopened and corrected. If a correction removes required data, completion can revert to incomplete. Derived metrics recalculate.

## Completed workout
After Finish Workout, active mutation affordances disappear and Story 42A/42B review rules apply.

## Acceptance Criteria
- [ ] untouched exercise never completes
- [ ] valid committed exercise completes deterministically
- [ ] successful completion auto-collapses details
- [ ] completion feedback is noticeable but non-blocking
- [ ] moving to another exercise can collapse prior details
- [ ] invalid data is never auto-completed
- [ ] completed exercise can reopen during active workout
- [ ] edits recalculate completion and derived metrics
- [ ] required-data deletion can return exercise to incomplete
- [ ] active-complete and workout-complete review remain distinct
...
## Claude Steering
Completion must come from one canonical domain selector/function reused everywhere. Do not create competing completion logic in card, set editor, summary, API mapping, and progress.
