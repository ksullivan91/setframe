# Story 42.2 — Replace Generic Accordion With a Controlled Exercise Work Card
...
## User Story
As a user logging on my phone, I want a compact fast-entry surface with optional detailed sets, so I only open the full form when I need it.

## Product Intent
Replace the product-level accordion with an `ExerciseWorkCard` backed by a disclosure primitive.

### Preferred web primitive
Evaluate React Aria `Disclosure`/`DisclosureGroup` first. Radix is a secondary choice if repository architecture strongly favors it.

## Interaction Contract
Only the dedicated chevron toggles details. These must never toggle details:
- weight/reps/duration/distance/RPE inputs
- quick-log CTA
- ellipsis/context menu
- copy/delete/edit controls

At most one detailed exercise panel should be open in the normal active-workout flow. Opening another detail panel collapses the previous one.

## Component anatomy

```text
ExerciseWorkCard
├── Identity + plan + progress
├── DisclosureControl
├── QuickLogArea
├── ContextAction
└── DetailPanel
...
## Expanded-state redesign
- neutral/white editable surface
- clear visual boundary between summary and detailed sets
- no full green wash over editable forms
- planned badges remain visible
- detail mode should read as an intentional editor, not “accordion content”

## Acceptance Criteria
- [ ] Product code no longer treats the whole exercise header as one accordion trigger.
- [ ] Dedicated disclosure control always visible.
- [ ] Input focus never toggles detail state.
- [ ] Context actions never toggle detail state.
- [ ] Expansion is controlled by app state.
- [ ] Opening another detailed exercise closes the previous detailed exercise.
- [ ] Quick log works while details remain collapsed.
- [ ] Completed active exercises can be reopened.
- [ ] Keyboard/screen-reader disclosure behavior is correct on web.
- [ ] Native mobile mirrors behavior with native semantics.
...
## Claude Steering
Use the library for semantics/state/focus, not its visual demo. The Setframe component should be named for the product domain (`ExerciseWorkCard`), not the underlying primitive.
