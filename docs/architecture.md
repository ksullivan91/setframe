...
| Imported health metrics (steps, HR, weight if imported, activity rings) | HealthKit | DB stores normalized snapshot + provenance, not raw samples. **Exception:** the heart-rate series of a workout attached to a session is stored as arrays keyed by (workout, kind) — see ADR 0012. A rolling daily metric is a cache; a finished workout is evidence. |
